...
| **ScalePad to CloudRadial Alignment** | [`scalepad-cloudradial-alignment/`](scalepad-cloudradial-alignment/) | Agent + workflow that maps a client's ScalePad Lifecycle Manager data to its CloudRadial home, grounded on the migration map in Knowledge. Plans first; writes only approved changes. |
