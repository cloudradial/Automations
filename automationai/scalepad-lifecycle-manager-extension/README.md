...
| File | Type | Role |
|---|---|---|
| [`lifecycle-manager.extension.yml`](lifecycle-manager.extension.yml) | `automationsExtension` | Slug `lifecycle-manager`, version 1.2.0 — 21 tools, 6 skills. Built from the catalog 1.0.0 export, so every existing tool and skill is unchanged apart from paging. |
...
- **Hardware type documented** on `scalepad_list_hardware_assets`: `WORKSTATION`, `SERVER`, `VIRTUAL`, `NETWORK`, `MOBILE`, `IMAGING` — filter with `filter[type]` to separate workstations from servers.
- **New skills:** *Inventory Installed Software*, *Review Assessments*.

## Install

1. **Extensions → Custom → Import** `lifecycle-manager.extension.yml`. Importing a custom extension with the catalog's slug overrides the catalog copy in that tenant (the same way the Hudu and CloudRadial Companies fixes are carried locally).
